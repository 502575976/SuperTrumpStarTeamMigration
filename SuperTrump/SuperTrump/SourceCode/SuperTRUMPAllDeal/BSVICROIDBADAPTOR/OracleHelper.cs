...
using SuperTRUMPCommon;
//using System.Transactions;

namespace BSVICROIDBADAPTOR.OracleHelper
{
    /// <summary>
    ...
    public sealed class OracleHelper
    {
        ...
        private OracleConnection objOracleDBConn;
        ...
        //private void OracleDBConnect()
        //{
        //    //open the connection to Oracle svr db
        //    objOracleDBConn = new OleDbConnection();
        //    objOracleDBConn.ConnectionString = strConnect;
        //    objOracleDBConn.Open();

        //}

        private void OracleDBConnect()
        {
            log4net.ILog STLogger;
            BSVICROICommon objLogConfig = new BSVICROICommon();
            STLogger = objLogConfig.SetLog4Net();
            try
            {
                STLogger.Debug("START:  " + DateTime.Now + " Tracing process within method in Adapter: " + MethodInfo.GetCurrentMethod().Name);
                //open the connection to Oracle svr db
                objOracleDBConn = new OracleConnection();
                objOracleDBConn.ConnectionString = strConnect;
                if (objOracleDBConn.State == ConnectionState.Open)
                {
                    objOracleDBConn.Close();
                    objOracleDBConn.Open();
                }
                else
                {
                ...
                STLogger.Debug("END:  " + DateTime.Now + " Tracing process within method in Adapter: " + MethodInfo.GetCurrentMethod().Name);
            }
            catch(Exception Ex)
            {
                STLogger.Error(" Method Name:- " + System.Reflection.MethodInfo.GetCurrentMethod().Name + " Error Desc:- " + Ex.Message);
                throw; 
            }
            finally
            {
        ...
        private void OracleDBDisConnect()
        {

            //close the connection to the db if it is not closed
            if (objOracleDBConn.State != ConnectionState.Closed)
            {
                objOracleDBConn.Close();
                objOracleDBConn.Dispose();
            }
        ...
        public long ExecuteNonQuery(string astrSQL)
        {
            log4net.ILog STLogger;
            BSVICROICommon objLogConfig = new BSVICROICommon();
            STLogger = objLogConfig.SetLog4Net();
            OleDbCommand lobjOracleCmd = default(OleDbCommand);
            long llngAffectedRows = 0;
            ...
            try
            {
                ...
                OracleDBConnect();
                lobjOracleCmd = new OleDbCommand(astrSQL);
                //lobjOracleCmd.Connection = objOracleDBConn;
            ...
            {
                STLogger.Error(" Method Name:- " + System.Reflection.MethodInfo.GetCurrentMethod().Name + " Error Desc:- " + Excep.Message);
        ...
        public string ExecuteNonQueryWithMsg(string astrSQL)
        {
            log4net.ILog STLogger;
            BSVICROICommon objLogConfig = new BSVICROICommon();
            STLogger = objLogConfig.SetLog4Net();
            OracleCommand lobjOracleCmd = default(OracleCommand);
            long llngAffectedRows = 0;
            ...
            OracleDBConnect();
            OracleTransaction Trans = objOracleDBConn.BeginTransaction();
            lobjOracleCmd = new OracleCommand(astrSQL);
            lobjOracleCmd.Connection = objOracleDBConn;
            lobjOracleCmd.CommandType = CommandType.Text;
            lobjOracleCmd.CommandTimeout = 0;
            lobjOracleCmd.Transaction = Trans;
            try
            {
                llngAffectedRows = lobjOracleCmd.ExecuteNonQuery();
                Trans.Commit();
                STLogger.Debug("END:  " + DateTime.Now + " Tracing process within method in Adapter: " + MethodInfo.GetCurrentMethod().Name);
                return llngAffectedRows.ToString();
        ...
        public static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
            ...
            using (OracleConnection cn = new OracleConnection(connectionString))
            {
                //cn.Open(); //commented by gaurav on 18-05-2005
        ...
        /// <summary>
        /// Added on 03-11-10 to insert bulk data in database.
        /// </summary>
        /// <param name="strConnection">Connection String</param>
        /// <param name="ds">Dataset object</param>
        /// <param name="strMappingTableName">Table Name to map with database for bulk insertion</param>
        /// <returns>No of Rows changed</returns>
        public static Int32 ExecuteDataset(OleDbConnection strConnection, DataSet ds, string strMappingTableName)
        {
            OleDbConnection conn = strConnection;
            ...
            System.Data.OleDb.OleDbDataAdapter objAdap = new System.Data.OleDb.OleDbDataAdapter();
            objAdap.SelectCommand = new System.Data.OleDb.OleDbCommand("Select * from " + strMappingTableName, conn);
            System.Data.OleDb.OleDbCommandBuilder objBldr = new System.Data.OleDb.OleDbCommandBuilder(objAdap);
            objAdap.Fill(ds, "PMSData");
            DataTable dtPMS = new DataTable();
            dtPMS = ds.Tables[strMappingTableName];
            string strInsert = "BEGIN" + Environment.NewLine;
            foreach (DataRow dr in dtPMS.Rows)
            {
                dr.SetAdded(); // sets its RowState to "Added"    
                strInsert += "Insert Into TBL_PMSDATA(PMS_Location, EOT_UPSIDE, ST_PRODUCT_NAME) Values('" + dr[0] + "','" + dr[1] + "','" + dr[2] + "');" + Environment.NewLine;
            }
            strInsert += "END;";
            //return objAdap.Update(dtPMS);
            OleDbCommand cmd = new OleDbCommand(strInsert, conn);
            cmd.ExecuteNonQuery();
            return 0;
        }

        private static void UploadDatabase(string strDATPath, string strLoaderFile, string strLogFile)
        {
            //strDATPath = @"C:\WorkingFolder\DataOut.DAT";
            //strLoaderFile = @"C:\WorkingFolder\Ctrl.ctl";
            //strLogFile = @"c:\WorkingFolder\dataFile.log";
            string strCmd, strSQLLoader;
            strCmd = "sqlldr VIC_ROI/VIC_ROI@MMFD01 control=" + strLoaderFile + " LOG=" + strLogFile;
            System.IO.DirectoryInfo di;
            try
            {
                System.Diagnostics.ProcessStartInfo cmdProcessInfo = new System.Diagnostics.ProcessStartInfo("cmd.exe");
                di = new System.IO.DirectoryInfo(strDATPath);
                strSQLLoader = "";
                strSQLLoader += "LOAD DATA INFILE '" + strDATPath.ToString().Trim() + "' APPEND INTO TABLE TBL_PMSDATA FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"'(PMS_LOCATION,EOT_UPSIDE,ST_PRODUCT_NAME)";
                System.IO.StreamWriter writer = new System.IO.StreamWriter(strLoaderFile);
        ...
        public static string TestServiceinADP()
        {
            return "Adp Successfully";
