CREATE OR REPLACE PROCEDURE UpdateStreamTableData IS
CURSOR bd_cur IS
 ...
 aa_binary(84):=Yr6_Dec ;	
			 ...
			 FOR i IN 1 .. 84 LOOP
			  if Prev <> aa_binary(i)  then				 		  	 
				 ...
				 if blnFlag = true then    -- for first occurance
					 blnflag:= false;
					 Prev:=aa_binary(i);
					 Totalcount:=1;				 
				 else
				 
					  if i=84 and aa_binary(i) = '0.00' then  -- for last occurance
					 	 Totalcount:=1;
						 exit;
					  else
