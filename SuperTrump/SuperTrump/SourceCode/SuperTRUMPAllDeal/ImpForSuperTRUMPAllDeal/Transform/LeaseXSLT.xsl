        ...
        <AD_HOC_QUERY>
          <SuperTRUMP>
            <Transaction query="QUI_001" id="TRAN1">
              ...
              <TransactionStartDate>
                <xsl:if test="AccountScheduleFeed/TRANSACTION_START_DATE!=''">
                  <!--<xsl:value-of select="substring(AccountScheduleFeed/TRANSACTION_START_DATE,7,4)" />-<xsl:value-of select="substring(AccountScheduleFeed/TRANSACTION_START_DATE,1,2)" />-<xsl:value-of select="substring(AccountScheduleFeed/TRANSACTION_START_DATE,4,2)" />-->
              ...
              <CommencementDate>
                <xsl:if test="AccountScheduleFeed/COMMENCEMENT_DATE!=''">
                  ...
                  <!--<xsl:value-of select="ms:format-date(AccountScheduleFeed/COMMENCEMENT_DATE, 'yyyy-MM-dd')" />-->
                </xsl:if>
              </CommencementDate>
              <Features>
              ...
              <xsl:choose>
                <xsl:when test="AccountScheduleFeed/PRODUCT='MEREG' or AccountScheduleFeed/PRODUCT='MENQSI' or AccountScheduleFeed/PRODUCT='MEQMUN' or AccountScheduleFeed/PRODUCT='MEOQSI' or AccountScheduleFeed/PRODUCT='MEGMUN'">
                  <LendingLoans>
                    <LendingLoan index="{position()-1}">
                      <CashflowSteps>
                        ...
                        <xsl:choose>
                          <!--<xsl:when test="//NewDataSet/AccountScheduleFeed/COMMENCEMENT_DATE!=//NewDataSet/AccountScheduleFeed/TRANSACTION_START_DATE">-->
                      ...
                      <Asset index="{position()-1}">
                        ...
                        <DeliveryDate>
                          <xsl:if test="DATE_IN_SERVICE!=''">
                            <!--<xsl:value-of select="substring(DATE_IN_SERVICE,7,4)" />-<xsl:value-of select="substring(DATE_IN_SERVICE,1,2)" />-<xsl:value-of select="substring(DATE_IN_SERVICE,4,2)" />-->
                        ...
                        <FundingDate>
                          <xsl:if test="FUNDING_DATE!=''">
                            <!--<xsl:value-of select="substring(FUNDING_DATE,7,4)" />-<xsl:value-of select="substring(FUNDING_DATE,1,2)" />-<xsl:value-of select="substring(FUNDING_DATE,4,2)" />-->
              ...
              <xsl:choose>
                <xsl:when test="AccountScheduleFeed/PRODUCT!='MEREG' and AccountScheduleFeed/PRODUCT!='MENQSI' and AccountScheduleFeed/PRODUCT!='MEQMUN' and AccountScheduleFeed/PRODUCT!='MEOQSI' and AccountScheduleFeed/PRODUCT!='MEGMUN'">
                  <Rents>
			...
			<xsl:value-of select="substring(substring-after(substring-after(AccountScheduleFeed/BOOKING_DATE,'/'),'/'),1,4)" />-<xsl:value-of select="substring-before(AccountScheduleFeed/BOOKING_DATE,'/')" />-<xsl:value-of select="substring-before(substring-after(AccountScheduleFeed/BOOKING_DATE,'/'),'/')" />
              ...
              <Writefile>
                  <xsl:attribute name="filename">E:\internalsites\SuperTRUMP\PRMTemplates\TestPRM\<xsl:value-of select="AccountScheduleFeed/ACCOUNT_SCHEDULE_NBR"/>.prm</xsl:attribute>
                </Writefile>
            </Transaction>
          </SuperTRUMP>
        </AD_HOC_QUERY>
